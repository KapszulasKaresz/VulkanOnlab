...
void Texture2D::generateCheckered(glm::vec3& color1, glm::vec3& color2, float scale)
{
	texWidth = 1024;
	texHeight = 1024;
...
void Texture2D::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
void Texture2D::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = Application::beginSingleTimeCommands(); 
